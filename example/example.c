...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    int log_json;
    if (tomlinc_get_bool_value(toml_file, "general", "log_json", &log_json) == 0) {
        printf("[general] get log_json: %s\n", log_json ? "true" : "false");
    } else {
        printf("[general] Failed to get  log_json.\n");
    }
    log_json = true;
    if (tomlinc_set_bool_value(toml_file, "general", "log_json", log_json) == 0) {
    ...
    */
    ...
    if (tomlinc_set_string_value(toml_file, "logging", "level", set_logging_level) == 0) {
    ...
    int json;
    if (tomlinc_get_bool_value(toml_file, "logging", "json", &json) == 0) {
        printf("[logging] get json: %s\n", json ? "true" : "false");
    } else {
        printf("[logging] Failed to get json.\n");
    }
    json = true;
    if (tomlinc_set_bool_value(toml_file, "logging", "json", json) == 0) {
    ...
    */
    ...
    if (tomlinc_get_int_value(toml_file, "sqlite", "max_open_connections", &max_open_connections) == 0) {
        printf("[sqlite] get max_open_connections: %d\n", max_open_connections);
    } else {
        printf("[sqlite] Failed to get max_open_connections.\n");
    }
    max_open_connections = 10;
    if (tomlinc_set_int_value(toml_file, "sqlite", "max_open_connections", max_open_connections) == 0) {
    ...
    */
    ...
    if (enabled_array) {
        size_t array_size = 0;
        if (tomlinc_get_array_size(enabled_array, &array_size) != 0) {
            fprintf(stderr, "[integration] Failed to get array size\n");
            return -1;
        }
        ...
        for (size_t i = 0; i < array_size; i++) {
            ...
            if (tomlinc_array_value_is_string(enabled_array, i)) {
                const char *value = tomlinc_array_get_string(enabled_array, i);
                if (value) {
                    printf("\"%s\"", value);
                } else {
                    printf("Error: Invalid string at index %zu", i);
                }
            } else if (tomlinc_array_value_is_int(enabled_array, i)) {
                int value;
                if (tomlinc_array_get_int(enabled_array, i, &value) == 0) {
                    printf("%d", value);
                } else {
                    printf("Error: Invalid integer at index %zu", i);
                }
            } else if (tomlinc_array_value_is_float(enabled_array, i)) {
                float value;
                int precision;
                if (tomlinc_array_get_float(enabled_array, i, &value, &precision) == 0) {
                    printf("%.*f", precision, value);
                } else {
                    printf("Error: Invalid float at index %zu", i);
                }
            } else if (tomlinc_array_value_is_bool(enabled_array, i)) {
                int value;
                if (tomlinc_array_get_bool(enabled_array, i, &value) == 0) {
                    printf("%s", value ? "true" : "false");
                } else {
                    printf("Error: Invalid boolean at index %zu", i);
                }
            } else {
                printf("Error: Unknown type at index %zu", i);
            }
        }
        printf("]\n");
    } else {
    ...
    if (tomlinc_array_set_value(toml_file, "integration", "enabled", 0, (void *)new_string, TOML_VALUE_STRING) == 0) {
        printf("[integration] enabled: %s\n", new_string);
    } else {
        printf("[integration] Failed to update enabled\n");
    }
    if (tomlinc_array_add_value(toml_file, "integration", "enabled", "mqtt", TOML_VALUE_STRING) == 0) {
        printf("[integration] enabled: %s\n", "mqtt");
    } else {
        printf("[integration] Failed to update enabled\n");
    }
    char *new_value_string = "mysql";
    if (tomlinc_array_add_value(toml_file, "integration", "enabled", new_value_string, TOML_VALUE_STRING) == 0) {
        printf("[integration] enabled: %s\n", new_value_string);
    } else {
        printf("Failed to add 'mysql' to the array.\n");
    }
    int new_value_int = 42;
    if (tomlinc_array_add_value(toml_file, "integration", "enabled", &new_value_int, TOML_VALUE_INT) == 0) {
        printf("[integration] enabled: %d\n", new_value_int);
    } else {
        printf("Failed to add 42 to the array.\n");
    }
    float new_value_float = 3.1234f;
    if (tomlinc_array_add_value(toml_file, "integration", "enabled", &new_value_float, TOML_VALUE_FLOAT) == 0) {
        printf("[integration] enabled: %.*f\n", 4, new_value_float);
    } else {
        printf("Failed to add 3.12345 to the array.\n");
    }
    int new_value_bool = true;
    if (tomlinc_array_add_value(toml_file, "integration", "enabled", &new_value_bool, TOML_VALUE_BOOL) == 0) {
    ...
    */
    ...
    if (mixed_array) {
        size_t array_size = 0;
        if (tomlinc_get_array_size(mixed_array, &array_size) != 0) {
            fprintf(stderr, "[integration.settings] Failed to get array size\n");
            return -1;
        }
        ...
        for (size_t i = 0; i < array_size; i++) {
            ...
            if (tomlinc_array_value_is_string(mixed_array, i)) {
                const char *value = tomlinc_array_get_string(mixed_array, i);
                if (value) {
                    printf("\"%s\"", value);
                } else {
                    printf("Error: Invalid string at index %zu", i);
                ...
                int value;
                if (tomlinc_array_get_int(mixed_array, i, &value) == 0) {
                    printf("%d", value);
                } else {
                    printf("Error: Invalid integer at index %zu", i);
                }
                ...
                float value;
                int precision;
                if (tomlinc_array_get_float(mixed_array, i, &value, &precision) == 0) {
                    printf("%.*f", precision, value);
                } else {
                    printf("Error: Invalid float at index %zu", i);
                }
                ...
                int value;
                if (tomlinc_array_get_bool(mixed_array, i, &value) == 0) {
                    printf("%s", value ? "true" : "false");
                } else {
                    printf("Error: Invalid boolean at index %zu", i);
                }
    ...
    if(tomlinc_array_set_value(toml_file, "integration.settings", "mixed", 0, new_string_value, TOML_VALUE_STRING) == 0) {
        printf("[integration.settings] set mixed[0]: %s\n", new_string_value);
    } else {
        printf("[integration.settings] Failed to update mixed\n");
    }
    int new_int_value = 2;
    if(tomlinc_array_set_value(toml_file, "integration.settings", "mixed", 1, &new_int_value, TOML_VALUE_INT) == 0) {
    ...
    if(tomlinc_array_set_value(toml_file, "integration.settings", "mixed", 2, &new_float_value, TOML_VALUE_FLOAT) == 0) {
    ...
    if(tomlinc_array_set_value(toml_file, "integration.settings", "mixed", 3, &new_bool_value, TOML_VALUE_BOOL) == 0) {
    ...
    */
    ...
    int enabled;
    if (tomlinc_get_bool_value(toml_file, "integration.mqtt.client", "enabled", &enabled) == 0) {
        printf("[integration.mqtt.client] get enabled: %s\n", enabled ? "true" : "false");
    } else {
        printf("[integration.mqtt.client] Failed to get enabled.\n");
    }
    enabled = false;
    if (tomlinc_set_bool_value(toml_file, "integration.mqtt.client", "enabled", enabled) == 0) {
    ...
    */
    ...
    int frequency;
    if (tomlinc_get_int_value(toml_file, "backend.basic_station", "frequency", &frequency) == 0) {
        printf("[backend.basic_station] get frequency: %d\n", frequency);
    } else {
        printf("[backend.basic_station] Failed to get frequency\n");
    }
    frequency = 928000000;
    if (tomlinc_set_int_value(toml_file, "backend.basic_station", "frequency", frequency) == 0) {
    ...
    if (tomlinc_save_file(toml_file, argv[2]) == 0) {
        printf("TOML file updated successfully\n");
    } else {
        printf("Failed to update TOML file\n");
    }
